...
public class AppTest {
	...
	public AppTest(String username, String password, String expected, int rowNum) {
	...
	public void addUserAttempt() throws IOException, InterruptedException {
		...
		driver.get(Constant.WEBPAGE);
		EntryPage entryPage = PageFactory.initElements(driver, EntryPage.class);
		entryPage.clickRegister();
		
		RegisterPage registerPage = PageFactory.initElements(driver, RegisterPage.class);
		registerPage.addUser(username,  password);
		String output = registerPage.checkOutput();
		...
		try {
			assertEquals("User not added.", expected, output);
			cell = row.getCell(4);
			if (cell == null) {
